...
`config/agent-context-deep.md` contains richer background loaded only when materially useful.
...
V1.2 adds an optional provider-agnostic Guardian Angel review loop.
...
`DEADLINE_MODE=true` deterministically selects fast cadence unless paused. V1.2 does not automatically infer approaching Issue deadlines.
...
A Rep V1.2 is the current V1 release. See `CURRENT.md` and `CHANGELOG.md`.
