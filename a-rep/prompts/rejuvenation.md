...
6. Research or experiment when useful. New or changed material belongs in `scratch/` while it is untrusted.
7. Test proposed scripts or procedures as far as the current environment reasonably permits and record actual evidence rather than claims.
8. Record worthwhile proposals and outcomes in Issue 17, and use Issues 13, 14, or 15 for their corresponding backlogs.
9. Do not promote material into `procedures/` unless the required review and human approval are already durably present.
10. If important productive work has become urgent, do not compete with it. Record anything useful and exit.
11. Exit cleanly when the useful rejuvenation work for this cycle is complete.

Respect the same one-PRIMARY lease as heartbeat work.

Guardian Angel suggestions are advisory evidence, not authority and not automatic procedure promotion.
...
Raw launcher output belongs under Git-ignored `.arep/raw-logs/`. Do not expose secrets in repository state, Issue comments, durable logs, or raw logs.
