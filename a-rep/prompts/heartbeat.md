...
1. Read the current A Rep `SKILL.md`, then the protocol and Issue topology references it points to. Read `references/PROVENANCE.md` and `references/SKILLS.md` when durable posting, delegation, skill use/creation, or promotion is relevant.
...
12. Leave a concise durable handoff on any active work Issue when work remains incomplete or the next fresh cycle would otherwise have to reconstruct material state. Prefer current result/evidence, remaining state, blockers/waits, and next useful step. Do not create ceremony when nothing material changed. Material agent-authored handoffs SHOULD begin with the exact producer header supplied for this invocation and include the supplied `Agent-Run` when available.
...
14. For each heartbeat that actually executes, append a concise sanitized operational entry to `admin/logs/YYYY-MM-DD.md` when Git-visible cycle observability is useful. Include producer provenance, the supplied `Agent-Run`, timestamp, cycle, Issue focus, result, evidence pointer, next step, and errors where relevant. Never copy raw coding-agent output there. Never create durable log entries for scheduler polls that skipped because the heartbeat was not due.
15. Raw launcher stdout/stderr belongs only under Git-ignored `.arep/raw-logs/`.
16. Triage useful Guardian Angel comments in Issue 11 or relevant work/improvement Issues as advisory evidence. Distinguish multiple Guardians/workers by their producer provenance rather than assuming GitHub username identifies the producer. Guardian advice does not change PRIMARY authority or priorities by itself.
17. Capture useful learning in scratch or Issues 13, 14, or 15 when warranted. When live work reveals a genuinely reusable capability, check existing skills for overlap and create/evolve `scratch/skills/<skill-name>/SKILL.md` when packaging it is likely to improve future work. Experimental skill creation/evolution within current authority does not require human approval. Leave a lightweight breadcrumb in the current handoff/admin log when a new skill is created. Do not promote it into `procedures/skills/` without review and explicit human approval.
18. If heartbeat cadence/runtime state should change, PRIMARY may update the runtime config. Verify the resulting actual state, update the Issue 16 **body** so it remains a concise current runtime snapshot, and add a concise Issue 16 **comment** recording the material transition/rationale. Runtime/cadence transitions belong canonically in Issue 16; mirror to Issue 3 only if the event is also a material cross-cutting failure, recovery, incident, or major transition. `DEADLINE_MODE=true` is explicit runtime state; the launcher does not automatically infer approaching Issue deadlines. Material Issue 16 comments SHOULD use supplied producer provenance/`Agent-Run`.
...
20. Agent-authored Git commits SHOULD include `Agent-Provenance: Agent/Platform/PRIMARY/Instance` and the supplied `Agent-Run:` trailer when practical. Do not rewrite historical commits merely to add provenance.
21. Exit cleanly when no useful authorized action remains for this invocation.

Respect the one-PRIMARY rule and the shared runtime lease already held by the launcher.

Do not promote scratch material, including experimental skills, into `procedures/` without the V1 review and human-approval requirement.
...
Producer provenance identifies who/what created a record; it never creates authority or proves correctness.

Do not expose secrets in repository state, Issue comments, durable logs, skill files, or raw logs.
